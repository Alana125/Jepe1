...
- Inscrição de projetos por alunos.
- Cadastro de professor orientador para cada projeto.
- Edição do projeto apenas pelo responsável enquanto o período de inscrição estiver aberto.
...
RB13. O professor orientador pode visualizar o projeto que orienta, mas não pode avaliá-lo.
...
RF-1: O sistema deve permitir que um aluno crie um projeto informando título, área, resumo, objetivos, metodologia, professor orientador e entre 2 e 5 integrantes, incluindo o responsável.
- Rastreado por: RB01, RB02, RB03, RB11, RB12

RF-2: O sistema deve permitir que o responsável edite os dados do projeto enquanto o período de inscrição estiver aberto.
- Rastreado por: RB02, RB03
...
- Rastreado por: RB02, RB05, RB06, RB08, RB09, RB13, RB14
...
RF-11: O sistema deve permitir que o aluno selecione um professor cadastrado como orientador do projeto.
- Rastreado por: RB11, RB12
...
- HU-1 (Aluno — Inscrever projeto): Como aluno, quero inscrever um projeto com 2–5 integrantes, professor orientador e área temática para participar da JEPE.
  - Aceitação: RF-1, RB01, RB11, RB12, campos obrigatórios, projeto criado como `Inscrito`.

- HU-2 (Aluno — Editar projeto): Como responsável pela inscrição, quero editar o projeto enquanto o prazo de inscrição estiver aberto.
  - Aceitação: RF-2, apenas responsável pode editar, bloqueio depois do período.

- HU-3 (Aluno — Consultar avaliação): Como aluno, quero consultar as avaliações e a nota final do meu projeto após a fase de avaliação.
  - Aceitação: RF-4, RF-9, acesso a avaliações e nota final apresentadas.
...
- HU-5 (Professor Orientador — Consultar projeto): Como professor orientador, quero visualizar os projetos que oriento para acompanhar sua participação na JEPE.
  - Aceitação: RF-12, RB13.
...
- RF-1: Projeto criado com área, professor orientador, 2–5 integrantes e status `Inscrito`.
- RF-2: Edição permitida apenas enquanto o período de inscrição estiver aberto e apenas pelo responsável.
- RF-3: Listagem permite filtro por área e situação e busca por título.
- RF-4: Avaliação salva e atualizada por mesmo professor; valores entre 0 e 10; proibição de avaliação de projeto orientado.
...
- RF-8: Cada perfil vê apenas ações autorizadas; professor só pode editar próprias avaliações durante fase aberta; professor orientador não pode avaliar projeto que orienta.
...
- RF-11: Apenas usuários com perfil de Professor podem ser selecionados como orientadores.
...
1. Inscrever Projeto
  - Ator: Aluno
  - Pré-condição: Autenticado como Aluno
  - Fluxo: Preencher formulário → selecionar área → selecionar professor orientador → adicionar integrantes → validar integrantes e orientador → salvar → `Inscrito`

2. Editar Projeto
  - Ator: Aluno (responsável)
  - Pré-condição: Período de inscrição aberto
  - Fluxo: Abrir projeto próprio → editar campos → salvar

3. Consultar Projeto Orientado
...
4. Avaliar Projeto
  ...
  - Pré-condição: Autenticado como Professor, fase de avaliação aberta, não ser orientador do projeto
  - Fluxo: Selecionar projeto → verificar conflito de interesse → preencher notas por critério → salvar ou atualizar

5. Consultar Avaliação e Nota Final
  - Ator: Aluno
  - Pré-condição: Projeto avaliado
  - Fluxo: Acessar detalhe do projeto → visualizar avaliações e nota final
...
7. Visualizar Ranking
  - Ator: Aluno, Professor ou Administrador
